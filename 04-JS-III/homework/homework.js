...
function dePalabrasAFrase(palabras) {
  ...
  for (let index = 0; index < palabras.length - 1; index++) {
    kdn = kdn + palabras[index] + " ";
  }
  return kdn + palabras[palabras.length - 1];
}
...
function mesesDelAño(array) {
  ...
  for (let index = 0; index < array.length; index++) {
    if (
      ...
      nuevorrglo.push(array[index]);
    }
  }
  if (nuevorrglo.length === 3) {
    return nuevorrglo;
  } else {
...
function breakStatement(numero) {
  ...
  for (let index = 0; index < 10; index++) {
    nuevorrglo.push((numero += 2));
    if (nuevorrglo[index] === index) {
